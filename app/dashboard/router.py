...
def get_dashboard_summary(
    ...
    total_study_sessions = db.query(func.count(models.StudySession.id)).filter(
    ...
    total_study_time = db.query(
        func.sum(
            func.julianday(func.coalesce(models.StudySession.end_time, func.current_timestamp())) - 
    ...
    user_flashcards = db.query(models.Flashcard).join(
        models.FlashcardSet,
        models.Flashcard.set_id == models.FlashcardSet.id
    ).filter(
        models.FlashcardSet.user_id == current_user.id
    ...
    for flashcard in user_flashcards:
        ...
        stats = models.FlashcardProgress.get_stats_by_flashcard(db, flashcard.id, current_user.id)
        
        if stats:
            if stats.correct_count and stats.correct_count > 2 and stats.average_difficulty and stats.average_difficulty < 2:
                mastered_cards += 1
...
def get_recent_activity(
    ...
    for session in recent_sessions:
        # Get the set title
        set_title = db.query(models.FlashcardSet.title).filter(
            models.FlashcardSet.id == session.set_id
        ).scalar() or "Unknown Set"
    ...
    for flashcard_set in recent_sets:
        ...
        card_count = db.query(func.count(models.Flashcard.id)).filter(
    ...
    recent_progress = db.query(models.FlashcardProgress).filter(
        models.FlashcardProgress.user_id == current_user.id
    ).order_by(models.FlashcardProgress.created_at.desc()).limit(limit).all()
    ...
    for progress in recent_progress:
        ...
        if flashcard:
            # Get the set title
            set_title = db.query(models.FlashcardSet.title).join(
                models.Flashcard,
                models.FlashcardSet.id == models.Flashcard.set_id
            ).filter(
                models.Flashcard.id == progress.flashcard_id
            ).scalar() or "Unknown Set"
...
def get_set_statistics(
    ...
    user_sets = db.query(models.FlashcardSet).filter(
        models.FlashcardSet.user_id == current_user.id
    ).all()
    ...
    for flashcard_set in user_sets:
        ...
        total_cards = db.query(func.count(models.Flashcard.id)).filter(
            models.Flashcard.set_id == flashcard_set.id
        ).scalar() or 0
        ...
        study_count = db.query(func.count(models.StudySession.id)).filter(
            models.StudySession.set_id == flashcard_set.id,
            models.StudySession.user_id == current_user.id
        ).scalar() or 0
        ...
        if total_cards > 0:
            ...
            for flashcard in flashcards:
                ...
                if stats and stats.correct_count and stats.correct_count > 2 and stats.average_difficulty and stats.average_difficulty < 2:
                    mastered_count += 1
            
            mastery_percentage = (mastered_count / total_cards) * 100
        
        set_stats.append(schemas.SetStatistics(
...
def get_study_time_distribution(
    ...
    for i in range(6, -1, -1):
        ...
        daily_minutes = db.query(
            func.sum(
                func.julianday(func.min(func.coalesce(models.StudySession.end_time, func.current_timestamp()), day_end)) - 
                func.julianday(func.max(models.StudySession.start_time, day_start))
            ) * 24 * 60  # Convert to minutes
        ).filter(
            models.StudySession.user_id == current_user.id,
            models.StudySession.start_time <= day_end,
            func.coalesce(models.StudySession.end_time, func.current_timestamp()) >= day_start
        ).scalar() or 0
        
        daily_data.append(schemas.TimePoint(
            date=date_str,
    ...
    for i in range(3, -1, -1):
        ...
        weekly_minutes = db.query(
            func.sum(
                func.julianday(func.min(func.coalesce(models.StudySession.end_time, func.current_timestamp()), week_end_dt)) - 
                func.julianday(func.max(models.StudySession.start_time, week_start_dt))
            ) * 24 * 60  # Convert to minutes
        ).filter(
            models.StudySession.user_id == current_user.id,
            models.StudySession.start_time <= week_end_dt,
            func.coalesce(models.StudySession.end_time, func.current_timestamp()) >= week_start_dt
        ).scalar() or 0
        
        weekly_data.append(schemas.TimePoint(
            date=week_str,
            minutes=float(weekly_minutes)
        ))
    ...
    for i in range(5, -1, -1):
        ...
        if month_date.month == 12:
            month_end = datetime(month_date.year + 1, 1, 1, 0, 0, 0) - timedelta(seconds=1)
        else:
            month_end = datetime(month_date.year, month_date.month + 1, 1, 0, 0, 0) - timedelta(seconds=1)
        
        monthly_minutes = db.query(
            func.sum(
                func.julianday(func.min(func.coalesce(models.StudySession.end_time, func.current_timestamp()), month_end)) - 
                func.julianday(func.max(models.StudySession.start_time, month_start))
            ) * 24 * 60  # Convert to minutes
        ).filter(
            models.StudySession.user_id == current_user.id,
            models.StudySession.start_time <= month_end,
            func.coalesce(models.StudySession.end_time, func.current_timestamp()) >= month_start
        ).scalar() or 0
        
        monthly_data.append(schemas.TimePoint(
            date=month_str,
            minutes=float(monthly_minutes)
        ))
